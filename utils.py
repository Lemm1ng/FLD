...
class CropRandom(object):
    ...
    def __call__(self, sample):
        img = sample[self.elem_name]
        h, w, _ = img.shape
        margin_h = np.random.randint(0, h - self.size)
        margin_w = np.random.randint(0, w - self.size)
        sample[self.elem_name] = img[margin_h:margin_h + self.size, margin_w:margin_w + self.size]
        sample["crop_margin_x"] = margin_w
        sample["crop_margin_y"] = margin_h
        ...
        with open(landmark_file_name, "rt") as fp:
            for i, line in tqdm.tqdm(enumerate(fp), total=num_lines + 1):
                ...
                if split in ("train", "val"):
                    ...
                    landmarks = np.array(landmarks, dtype=np.float).reshape((len(landmarks) // 2, 2))
                    self.landmarks.append(landmarks)

        if split in ("train", "val"):
            self.landmarks = torch.as_tensor(np.array(self.landmarks))
        else:
            self.landmarks = None
